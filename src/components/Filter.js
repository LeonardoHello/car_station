...
const Filter = ({ filterName, setFilterQuery }) => {
	...
	return (
		filterName === 'Make' ?
			<div className='MYP'>
				{vehicleMake.collection && vehicleMake.collection.map((elem, index) => <button key={index} onClick={() => setFilterQuery(`where make = '${elem.name}'`)}>{elem.name}</button>)}
			</div> : 
		filterName === 'Year' ?
			<div className='MYP filter_year'>
				{vehicleYears.sort().map((elem, index) => <button key={index} onClick={() => setFilterQuery(`where year = ${elem}`)}>{elem}</button>)}
			</div> :
		filterName === 'Price' ?
			<div className='MYP'>
				{['$1 - $99,999', '$100,000 - $249,999', '$250,000 - $499,999', '$500,000 - $999,999'].map((elem, index) => <button key={index} onClick={() => (
					setFilterQuery(`where price > ${
